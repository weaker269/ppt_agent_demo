...
"""
        ...
        try:
            response = await self.execute_with_retry(
                ...
                model=self.model,
                messages=[
                    {"role": "system", "content": "你是一个专业的文档分析师，擅长将文档内容结构化为适合制作演示文稿的章节。"},
...
"""
        ...
        try:
            response = await self.execute_with_retry(
                ...
                model=self.model,
                messages=[
                    {"role": "system", "content": "你是一个专业的演示文稿设计师，擅长创建清晰、有效的幻灯片内容。"},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.4,
                max_tokens=1500
...
"""
        ...
        try:
            response = await self.execute_with_retry(
                ...
                model=self.model,
                messages=[
                    {"role": "system", "content": "你是一个专业的演示文稿质量评估师，能够客观地评估幻灯片内容的质量。"},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.2,
                max_tokens=800
...
"""
        ...
        try:
            response = await self.execute_with_retry(
                ...
                model=self.model,
                messages=[
                    {"role": "system", "content": "你是一个专业的内容优化师，擅长根据反馈改进演示文稿内容。"},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.3,
                max_tokens=1500
...
"""
        ...
        try:
            response = await self.execute_with_retry(
                ...
                model=self.model,
                messages=[
                    {"role": "system", "content": "你是一个专业的演讲稿撰写师，擅长创作引人入胜的演讲内容。"},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.6,
                max_tokens=1000
...
"""
        ...
        try:
            response = await self.execute_with_retry(
                ...
                model=self.model,
                messages=[
                    {"role": "system", "content": "你是一个专业的演讲稿撰写师，擅长创作结构完整、内容丰富的演讲稿。"},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.5,
                max_tokens=3000
    ...
    def _estimate_cost(self, input_tokens: int, output_tokens: int) -> float:
        ...
        if "gpt-4" in self.model.lower():
            ...
            return (input_tokens / 1000 * 0.0015) + (output_tokens / 1000 * 0.002)
        else:
            return super()._estimate_cost(input_tokens, output_tokens)
